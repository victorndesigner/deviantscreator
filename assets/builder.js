document.addEventListener('DOMContentLoaded', () => {
    ...
    function updatePreview() {
        ...
        if (carouselNav) {
            carouselNav.style.display = (selectedDeviant.imgs && selectedDeviant.imgs.length > 1) ? 'flex' : 'none';
        }
    }
    ...
    function generateCanvas(clone) {
        html2canvas(clone, {
            backgroundColor: null, scale: 1, useCORS: true, allowTaint: true, logging: false, width: 1640, height: 1080
        }).then(canvas => {
            try {
